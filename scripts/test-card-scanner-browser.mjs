...
try {
  ...
  const photoBounds = await page.getByAltText('Photo de la carte à identifier', { exact: true }).boundingBox()
  await page.mouse.move(photoBounds.x + photoBounds.width * 0.45, photoBounds.y + photoBounds.height * 0.84)
  await page.mouse.down()
  await page.mouse.move(photoBounds.x + photoBounds.width * 0.98, photoBounds.y + photoBounds.height * 0.98, { steps: 5 })
  await page.mouse.up()
  await page.getByRole('button', { name: 'Lire la zone sélectionnée', exact: true }).click()
  await page.getByRole('heading', { name: '3. Choisis l’illustration exacte' }).waitFor({ timeout: 90_000 })
  assert.equal(await page.getByLabel('Numéro de carte', { exact: true }).inputValue(), 'OP01-001', 'Read a user-selected crop')
